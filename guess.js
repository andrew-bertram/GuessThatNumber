// GUESS THAT NUMBER
// Text/messsage to be used throughout file
...
while (restartGame){
    ...
    rangeNum = parseInt(prompt(`Please enter a maximum for the range:`));

    // Verifies the user's entry for a range number is a number greater than zero (NOTE: All numbers, positive and negative, have a default boolean value of true, except for zero which has a default boolean value of false. Also, NaN has a defualt boolean value of false as well.)
    while (!rangeNum || rangeNum < 1){
        range = parseInt(prompt(enterNumText));
    }
    
    // Cretes the random number using the range number entered by the user
    ...
    while (true){
        // Displays the number/answer when a codes
        if (guess === "shoot"){
        ...
        // Checks if the user guessed corretly. If so, the game ends
        if (guess === randomNum){
    ...
    // Prompts user with optio to play again 
